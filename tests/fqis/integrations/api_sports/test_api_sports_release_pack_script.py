...
from scripts.fqis_api_sports_release_pack import main
...
def test_release_pack_script_returns_non_zero_when_require_ready_and_blocked(tmp_path, capsys):
    code = main(
        [
            "--ledger",
...
def test_release_pack_script_writes_and_loads_pack(tmp_path, capsys):
    ...
    load_code = main(["--load", str(pack_path), "--require-ready"])
...
def test_release_pack_script_allows_warning_release(tmp_path, capsys):
    ledger_path = tmp_path / "run_ledger.jsonl"
    bundle_dir = tmp_path / "bundles"
    bundle_dir.mkdir()
    ...
    assert code == 0
    assert payload["status"] == "READY"
    assert payload["release_manifest"]["release_gate"]["status"] == "WARN"
