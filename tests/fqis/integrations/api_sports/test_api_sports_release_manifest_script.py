...
from scripts.fqis_api_sports_release_manifest import main
...
def test_release_manifest_script_returns_non_zero_when_require_ready_and_blocked(tmp_path, capsys):
    code = main(
        [
            "--ledger",
...
def test_release_manifest_script_writes_and_loads_manifest(tmp_path, capsys):
    ...
    load_code = main(["--load", str(output_path), "--require-ready"])
    loaded = json.loads(capsys.readouterr().out)

    assert load_code == 0
    assert loaded["release_id"] == payload["release_id"]


def test_release_manifest_script_allows_warning_release(tmp_path, capsys):
    ledger_path = tmp_path / "run_ledger.jsonl"
    bundle_dir = tmp_path / "bundles"
    bundle_dir.mkdir()
    ...
    assert code == 0
    assert payload["status"] == "READY"
    assert payload["release_gate"]["status"] == "WARN"
