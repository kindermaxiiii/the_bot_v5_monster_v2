...
from scripts.fqis_api_sports_release_gate import main
...
def test_release_gate_script_returns_non_zero_when_require_ready_and_blocked(tmp_path, capsys):
    code = main(
        [
            "--ledger",
...
def test_release_gate_script_passes_and_writes_output(tmp_path, capsys):
...
def test_release_gate_script_allows_warnings_without_audit_bundle(tmp_path, capsys):
    ledger_path = tmp_path / "run_ledger.jsonl"
    _write_ledger(ledger_path, [_ledger_entry("run-1")])
