...
class OUFTTranslator:
    def _group_quotes(self, quotes: Iterable[MarketQuote]) -> dict[tuple[str, float], dict[str, MarketQuote]]:
        ...
        for quote in quotes:
            if str(getattr(quote, "market_key", "")).upper() != "OU_FT":
                continue
            line = getattr(quote, "line", None)
            if line is None:
                continue
            key = (str(getattr(quote, "bookmaker", "") or ""), float(line))
            grouped.setdefault(key, {})[str(getattr(quote, "side", "")).upper()] = quote
        return grouped
    ...
    def _final_total_probability(
        self,
        probabilities: dict[int, float],
        predicate: Callable[[int], bool],
    ) -> float:
        return _clamp(sum(prob for final_total, prob in probabilities.items() if predicate(int(final_total))), 0.0, 1.0)
    ...
    def translate(
        ...
        for (bookmaker, line), pair in sorted(grouped_quotes.items(), key=lambda item: (item[0][1], item[0][0])):
            for side, quote, opposite_quote in (
                ...
                under_budget = self._allowed_additional_goals_before_under_loss(line, current_total)
                over_already_won = side == "OVER" and current_total > float(line)
                under_already_lost = side == "UNDER" and current_total > float(line)

                if side == "OVER":
                    raw_probability = self._final_total_probability(
                        probability_state.final_total_goal_probs,
                        lambda final_total: float(final_total) > float(line),
                    )
                    favorable_distance = float(goals_needed)
                    adverse_distance = 0.0
                    score_state_budget = goals_needed
                    resolution_pressure = goals_needed / max(probability_state.lambda_total_remaining, 0.05)
                    state_fragility = _clamp(
                        0.55 * intelligence.score_state_fragility
                        + 0.45 * _clamp(favorable_distance / max(probability_state.lambda_total_remaining, 0.25), 0.0, 2.0),
                        0.0,
                        1.0,
                    )
                    late_fragility = _clamp(
                        (intelligence.minute / 95.0) * _clamp(favorable_distance / 2.0, 0.0, 1.0),
                        0.0,
                        1.0,
                    )
                    early_fragility = _clamp(
                        (1.0 - intelligence.minute / 95.0) * _clamp(favorable_distance / 2.5, 0.0, 1.0),
                        0.0,
                        1.0,
                    )
                else:
                    ...
                    adverse_distance = float(max(0, under_budget + 1))
                    score_state_budget = under_budget
                    resolution_pressure = probability_state.lambda_total_remaining / max(under_budget + 1, 1)
                    state_fragility = _clamp(
                        0.55 * intelligence.score_state_fragility
                    ...
                    late_fragility = _clamp(
                        (intelligence.minute / 95.0) * _clamp(1.0 / max(under_budget + 1, 1), 0.0, 1.0),
                        0.0,
                        1.0,
                    )
                    early_fragility = _clamp(
                        (1.0 - intelligence.minute / 95.0) * _clamp(2.0 / max(under_budget + 1, 1), 0.0, 1.0),
                        0.0,
                        1.0,
                    )
                ...
                payload = {
                    ...
                    "goal_budget_under": under_budget,
                    "remaining_added_goal_probs": probability_state.remaining_added_goal_probs,
                    "final_total_goal_probs": probability_state.final_total_goal_probs,
                ...
                reasons = ["shared_probability_core", f"bookmaker={bookmaker}"]
